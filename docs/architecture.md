...
The installed app includes JWT/API key authentication, saved conversations, an admin panel, and PydanticAI/OpenAI integration. RAG, billing, teams, Redis and external monitoring are disabled. Sections below describe the broader template and are not a claim that all options are enabled.
