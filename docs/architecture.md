...
## AgentHarbor in this repository

`fastapi_gen/` implements the generator; `template/` contains Jinja source templates. The running product is `my_ai_app/`, with a Next.js frontend and FastAPI backend backed by PostgreSQL. Generated app changes do not automatically modify the generator.

Data flows from the browser through frontend API integration to FastAPI routes, services, repositories, and PostgreSQL. AI chat is handled by the backend agent layer using a server-side provider key; the browser must never receive that key. Routes validate input and enforce access, services own business rules, and repositories own SQL. Repositories flush; the session boundary owns commit/rollback. Database migrations live in `my_ai_app/backend/alembic/versions/`.

The installed app includes JWT/API key authentication, saved conversations, an admin panel, PydanticAI/OpenAI integration, and a feature-flagged customer-support pilot with company workspaces, knowledge retrieval and reviewed reply drafting. Billing, Redis and hosted monitoring remain disabled. See [the support decision](decisions/0004-support-pilot.md) for the bounded native implementation and [its runbook](runbooks/support-pilot.md). Sections below describe the broader template and are not a claim that all options are enabled.

See [decisions](decisions/README.md), [quality requirements](quality.md), and [runbooks](runbooks/README.md).
