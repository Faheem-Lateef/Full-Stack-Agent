...
async def test_regular_user_cannot_change_role(auth_client, mock_user_service, role):
    response = await auth_client.patch(f"{settings.API_V1_STR}/users/me", json={"role": role})
    assert response.status_code == 403
    mock_user_service.update.assert_not_awaited()
