...
async def readiness_probe(
    ...
    # LLM provider â€” config-only check (avoid spending money on a probe call).
    ...
    if llm_provider:
        ...
        api_key = (getattr(settings, key_field, "") or "").strip() if key_field else ""
        checks["llm"] = {
    ...
    # Determine overall health â€” only db + redis are critical for readiness.
