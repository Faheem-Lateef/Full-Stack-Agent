...
function errorMessage(e: unknown, fallback: string): string {
  ...
  if (code === "MEMORY_VERSION_CONFLICT") {
    return "This file changed. Copy your draft, cancel, then reopen the latest version before saving.";
  }
...
export function MemoryManager() {
  ...
  const handleSave = async () => {
    ...
    if (!isValidMemoryName(path)) {
      toast.error("Use a flat file name up to 80 characters including .md - letters, digits, dots and dashes, no folders.");
      return;
    }
    ...
    try {
      ...
      // Preserve the draft on every failure, including conflicting edits.
      void refresh();
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-baseline justify-between gap-3">
        <p className="text-foreground/55 text-xs">
          These notes help the assistant remember your preferences across conversations.
          Edit anything that&apos;s wrong, or delete what it shouldn&apos;t keep.
        </p>
        <Button
          size="sm"
