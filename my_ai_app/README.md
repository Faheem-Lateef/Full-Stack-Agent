...
└── commands/             # Click CLI commands (auto-discovered by `my_ai_app cmd …`)
...
The generated project ships a Click CLI exposed as `my_ai_app` (after `make install`):

```bash
my_ai_app server run --reload          # dev server
my_ai_app db upgrade                   # apply migrations
my_ai_app db migrate -m "message"      # create new migration
my_ai_app user create-admin            # interactive admin creation
```

Run `make help` for a categorized list, or `my_ai_app --help` for full CLI docs.
